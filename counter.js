...
http.createServer(function (req, res) {
	...
	if (id) {
		...
		if (userId) {
			var userKey = conf.redis.keyPrefix.userId + userId + ':' + id
			var userKeyToday = conf.redis.keyPrefix.userId + Date.today().toString('yyyy-MM-dd') + ':' + userId + ':' + id
			redis.multi()
				.getset(userKey, '1')
				.getset(userKeyToday, '1')
				.expire(userKey, conf.uniq.expireTime)
				.exec(function(err, replies) {
					...
					if (replies[1] != '1') {
						...
						multi.expire(userKeyToday, 86400) //24h
					}
